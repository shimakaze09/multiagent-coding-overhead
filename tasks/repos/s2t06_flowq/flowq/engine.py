...
class Engine:
    def __init__(self, workflow, handlers=None, *, max_parallel=None, clock=None, log=None):
        self.max_parallel = DEFAULT_MAX_PARALLEL if max_parallel is None else max_parallel
    ...
    def running(self):
        return sum(1 for r in self.runs.values() if r.state == RUNNING)
    ...
    def _start(self):
        ...
        for run in scheduler.pick(ready, self.max_parallel - self.running()):
            run.move(RUNNING)
            run.attempts += 1
            run.started_at = now
    ...
    def _finish(self):
        ...
        for run in sorted(done, key=lambda r: (r.finish_at, r.spec.key)):
            ...
            elif run.attempts < run.spec.retry.max_attempts:
                run.move(RETRY_WAIT)
                run.retry_at = now + run.spec.retry.delay(run.attempts)
