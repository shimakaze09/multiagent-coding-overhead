...
A line has exactly one of `debit` or `credit`, a positive amount. An entry's
debits and credits must be equal in every currency it uses. `Line.signed`
gives the line as one signed amount (debits positive).
