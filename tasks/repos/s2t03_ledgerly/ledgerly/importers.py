...
def parse_bank_csv(text, *, bank_account, contra_account, currency, dayfirst=False,
    ...
    for n, row in enumerate(csv.DictReader(io.StringIO(text)), start=1):
        ...
        if amount == 0:
            continue
        money = Money(abs(amount), currency)
        if amount > 0:
            lines = [Line(bank_account, debit=money), Line(contra_account, credit=money)]
        else:
            lines = [Line(bank_account, credit=money), Line(contra_account, debit=money)]
        entries.append(Entry(f"{id_prefix}-{n}", parse_date(row["date"], dayfirst), lines,
