...
Arithmetic keeps full precision. Amounts are rounded to cents
(ROUND_HALF_EVEN) only when asked (``rounded()``), and when a value is
...
class Money:
    ...
    def allocate(self, ratios):
        ...
        The parts are whole cents and always sum exactly to the rounded
        amount. Each part first gets the floor of its exact share; the cents
        left over go one each to the parts with the largest fractional
        ...
        units = int((abs(total.amount) / CENT).to_integral_value())
        whole = sum(ratios)
        ...
        for i in order[:leftover]:
            parts[i] += 1
        return [Money(Decimal(sign * p) * CENT, self.currency) for p in parts]
