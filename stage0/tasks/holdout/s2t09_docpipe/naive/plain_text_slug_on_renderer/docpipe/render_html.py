...
class HtmlRenderer:
    ...
    def visit_document(self, node):
        ...
        notes = self.render_footnotes(node)
        return body + ("\n" + notes if notes else "")

    def visit_heading(self, node):
        level = min(6, max(1, node.level + self.options.heading_offset))
        hid = self._ids.slug(plain_text(node.children))
        return f'<h{level} id="{escape(hid)}">{self.render_children(node)}</h{level}>'
    ...
    def render_footnotes(self, doc):
        if not doc.footnotes:
            return ""
        items = "".join(
            f'<li id="fn-{f.number}">{"".join(self.render_node(c) for c in f.children)} '
