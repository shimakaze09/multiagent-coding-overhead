...
class Document(Node):
    ...
    def __init__(self, children=None):
        ...
        self.footnote_defs = {}   # label -> inline nodes, first definition wins
        self.footnotes = []       # Footnote, numbered by first reference
        self.warnings = []
...
class FootnoteRef(Node):
    ...
    def __init__(self, label):
        super().__init__()
        self.label = label
        self.number = None        # assigned by transforms.number_footnotes
        self.occurrence = 0       # 1 for the first reference to a note, 2 for the next...
