...
def sale(entry_id, on, amount, currency="EUR", bank="1000"):
    ...
    return Entry(entry_id, on, [Line(bank, m), Line("4000", -m)])
...
def test_entries_must_balance():
    with pytest.raises(UnbalancedEntry):
        Entry("E-1", date(2024, 1, 1), [Line("1000", Money("10", "EUR")),
                                        Line("4000", Money("-9.99", "EUR"))]).validate()
    with pytest.raises(UnbalancedEntry):
        Entry("E-2", date(2024, 1, 1), [Line("1000", Money("10", "EUR"))]).validate()
    with pytest.raises(ValueError):
...
def test_income_statement():
    ledger = Ledger(chart())
    ledger.post(sale("S-1", date(2024, 1, 5), "100.00"))
    ledger.post(Entry("P-1", date(2024, 1, 7), [Line("6000", Money("40", "EUR")),
                                                Line("1000", Money("-40", "EUR"))]))
