...
Schema 3 (current) stores each line's signed amount as a decimal string.
Schema 2 (ledgerly 1.x) stored a positive "debit" or "credit" decimal string.
Files written by older versions must keep loading (docs/conventions.md).
"""
    ...
    return {
        ...
        "entries": [
            {"id": e.id, "date": e.on.isoformat(), "description": e.description,
             "lines": [{"account": l.account, "amount": str(l.amount.amount),
...
def _line(data, schema):
    if schema == 3:
        return Line(data["account"], Money(to_decimal(data["amount"]), data["currency"]),
                    data.get("memo", ""))
    side = "debit" if data.get("debit") is not None else "credit"
    money = Money(to_decimal(data[side]), data["currency"])
    return Line(data["account"], money if side == "debit" else -money, data.get("memo", ""))
