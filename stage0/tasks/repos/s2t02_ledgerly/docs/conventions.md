...
* Supported currencies: USD, EUR, GBP, CHF. Rounding is to cents, always
  `ROUND_HALF_EVEN` (banker's rounding).

## Allocation

`Money.allocate(ratios)` splits the rounded amount into whole cents:

1. every part gets the floor of its exact proportional share;
2. the cents left over are handed out one at a time to the parts with
