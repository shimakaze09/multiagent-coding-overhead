...
def _blocks(tokens, doc):
    ...
    while i < len(tokens):
        ...
        if t.kind == "blank":
            ...
            i += 1
        elif t.kind == "footnote_def":
            if t.label in doc.footnote_defs:
                doc.warnings.append(f"duplicate footnote definition [^{t.label}] ignored")
            else:
                doc.footnote_defs[t.label] = inline.parse(t.text)
            i += 1
        elif t.kind == "quote":
            lines = []
